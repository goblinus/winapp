...
import (
	...
	"github.com/goblinus/winapp/internal/adapters/redis"
...
func main() {
	...
	defer apiRedis.Close()

	userRepo := inmemory.NewUserRepository()
	taskRepo := redis.NewTaskRepository(apiRedis)

	userService := services.NewUserService(userRepo)
	taskSerivce := services.NewTaskService(taskRepo)

	app := httpRoutes.RegisterRoutes(userService, taskSerivce, apiRedis)
	app.Run(":8085")
}
